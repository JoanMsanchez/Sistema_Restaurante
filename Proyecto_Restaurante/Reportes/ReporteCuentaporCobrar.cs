...
{
    public partial class ReporteCuentaporCobrar : Form
    {
            ...
            try
            {
                ...
                string consulta = @"
                    SELECT
                        ...
                        o.saldo_pendiente AS monto_adeudado,
                        o.fecha_vencimiento
                    FROM orden o
                    INNER JOIN cliente c ON c.id_cliente = o.id_cliente
                    WHERE o.saldo_pendiente > 0
                      AND o.estado = 1                      -- (opcional) quítalo si no lo usas
                ...
                using (var cmd = new SqlCommand(consulta, conexion))
                {
                    using (var da = new SqlDataAdapter(cmd))
                    {
                        ...
                        dtgCxC.AutoGenerateColumns = true;
                        dtgCxC.DataSource = dt;

                        if (dtgCxC.Columns.Contains("id_orden"))
                            dtgCxC.Columns["id_orden"].Visible = false;

                        dtgCxC.Columns["No"].HeaderText = "No";
                        dtgCxC.Columns["cliente"].HeaderText = "Cliente";
                        dtgCxC.Columns["fecha_vencimiento"].HeaderText = "Fecha Vencimineto";
                        dtgCxC.Columns["monto_adeudado"].HeaderText = "Saldo Pendiente";

                        if (dtgCxC.Columns.Contains("fecha_vencimiento"))
                            dtgCxC.Columns["fecha_vencimiento"].DefaultCellStyle.Format = "dd/MM/yyyy";

                        if (dtgCxC.Columns.Contains("saldo_pendiente"))
                        {
                            dtgCxC.Columns["saldo_pendiente"].DefaultCellStyle.Format = "N2";
                            dtgCxC.Columns["saldo_pendiente"].DefaultCellStyle.Alignment =
            ...
            string rutaLogo = null)
        {
            ...
            string[] req = { "No", "Cliente", "fecha_vencimiento", "monto_adeudado", };
            ...
            try
            {
                ...
                // Título del reporte
                var titulo = sec.AddParagraph("Reporte de Cuentas por Cobra");
                ...
                header.Cells[3].AddParagraph("Saldo Pendiente");
                ...
                foreach (DataGridViewRow row in grid.Rows)
                {
                    ...
                    DateTime f = Convert.ToDateTime(row.Cells["fecha_vencimiento"].Value);
                    r.Cells[2].AddParagraph(f.ToString("dd/MM/yyyy"));

                    r.Cells[3].AddParagraph(Convert.ToString(row.Cells["monto_adeudado"].Value));
                ...
                string nombre = $"Reporte_de_Cuenta_por_Cobrar{DateTime.Now:dd-MM-yyyy_HH.mm}.pdf";
        ...
        private void txtNombre_TextChanged(object sender, EventArgs e)
        {
            ...
            try
            {
                // Usa la misma cadena de conexión del field 'conexion'
                ...
                dtgCxC.Columns["No"].HeaderText = "No";
                dtgCxC.Columns["cliente"].HeaderText = "Cliente";
                dtgCxC.Columns["fecha_vencimiento"].HeaderText = "Fecha Vencimiento";
                dtgCxC.Columns["monto_adeudado"].HeaderText = "Saldo Pendiente";

                if (dtgCxC.Columns.Contains("fecha_vencimiento"))
                    dtgCxC.Columns["fecha_vencimiento"].DefaultCellStyle.Format = "dd/MM/yyyy";

                if (dtgCxC.Columns.Contains("monto_adeudado"))
                {
                    dtgCxC.Columns["monto_adeudado"].DefaultCellStyle.Format = "N2";
                    dtgCxC.Columns["monto_adeudado"].DefaultCellStyle.Alignment =
                        DataGridViewContentAlignment.MiddleRight;
                }
            }
            catch (Exception ex)
