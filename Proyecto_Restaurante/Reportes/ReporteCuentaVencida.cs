...
{
    public partial class ReporteCuentaVencida : Form
    {
        ...
        private void Titulo_MouseDown_2(object sender, MouseEventArgs e)
        {
            ReleaseCapture();
            ...
            try
            {
                ...
                string consulta = @"
                    SELECT
                        ...
                        c.nombre                AS cliente,
                        o.fecha_vencimiento,
                        DATEDIFF(DAY, o.fecha_vencimiento, GETDATE()) AS dias_vencidos,
                        o.saldo_pendiente
                    FROM orden o
                    ...
                    WHERE o.saldo_pendiente > 0
                      ...
                      AND o.estado = 1
                    ORDER BY o.fecha_vencimiento, c.nombre;";
                ...
                dgvCuentaVencida.DataSource = dt;

                // Ocultar columnas de ID
                if (dgvCuentaVencida.Columns.Contains("id_orden"))
                    dgvCuentaVencida.Columns["id_orden"].Visible = false;

                // Cambiar encabezados (igual que tú)
                dgvCuentaVencida.Columns["No"].HeaderText = "#";
                dgvCuentaVencida.Columns["cliente"].HeaderText = "Cliente";
                dgvCuentaVencida.Columns["fecha_vencimiento"].HeaderText = "Fecha Vencimiento";
                dgvCuentaVencida.Columns["dias_vencidos"].HeaderText = "Días Vencidos";
                dgvCuentaVencida.Columns["saldo_pendiente"].HeaderText = "Saldo Pendiente";

                if (dgvCuentaVencida.Columns.Contains("fecha_vencimiento"))
                    dgvCuentaVencida.Columns["fecha_vencimiento"].DefaultCellStyle.Format = "dd/MM/yyyy";

                if (dgvCuentaVencida.Columns.Contains("saldo_pendiente"))
                {
                    dgvCuentaVencida.Columns["saldo_pendiente"].DefaultCellStyle.Format = "N2";
                    dgvCuentaVencida.Columns["saldo_pendiente"].DefaultCellStyle.Alignment =
                        DataGridViewContentAlignment.MiddleRight;
                }
                if (dgvCuentaVencida.Columns.Contains("dias_vencidos"))
                    dgvCuentaVencida.Columns["dias_vencidos"].DefaultCellStyle.Alignment =
            ...
            string rutaLogo = null)
        {
            ...
            string[] req = { "No", "cliente", "fecha_vencimiento", "dias_vencidos", "saldo_pendiente" };
            ...
            try
            {
                ...
                doc.Info.Title = "Reporte - Cuentas Vencidas";
                ...
                // Título del reporte
                var titulo = sec.AddParagraph("Reporte de Cuentas Vencidas");
                ...
                header.Cells[2].AddParagraph("Fecha Vencimiento");
                header.Cells[3].AddParagraph("Días Vencidos");
                header.Cells[4].AddParagraph("Saldo Pendiente");
                for (int i = 0; i < 5; i++)
                {
                ...
                foreach (DataGridViewRow row in grid.Rows)
                {
                    ...
                    if (DateTime.TryParse(Convert.ToString(row.Cells["fecha_vencimiento"].Value), out var fv))
                        r.Cells[2].AddParagraph(fv.ToString("dd/MM/yyyy"));
                    else
                        r.Cells[2].AddParagraph(Convert.ToString(row.Cells["fecha_vencimiento"].Value));

                    r.Cells[3].AddParagraph(Convert.ToString(row.Cells["dias_vencidos"].Value));
                    r.Cells[4].AddParagraph(FormatoN2(row.Cells["saldo_pendiente"].Value));
                }
                ...
                string nombre = $"Reporte_Cuentas_Vencidas_{DateTime.Now:dd-MM-yyyy_HH.mm}.pdf";
